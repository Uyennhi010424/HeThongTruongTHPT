...
export default function ScoreFollow() {
  const { students, selectedIndex, loading: studentsLoading, error: studentsError, selectStudent } = useParentStudents();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [subjectsRes, namHocRes] = await Promise.all([
          getMonHoc(),
          getNamHoc()
        ]);
        if (!active) return;
        ...
        const diemsRes = await getDiem({ hocSinhId: student.id }).catch(() => null);
  ...
  return (
    <div className="w-full min-h-screen bg-[#F8FAFC] pb-12">
      ...
      <div className="p-6 md:p-12 w-full flex flex-col gap-6">
        ...
        {studentsError && !error && <div className="bg-white rounded-xl border border-red-200 p-8 text-center text-red-600 font-medium shadow-sm">{studentsError}</div>}

        {!error && !studentsError && !loading && !studentsLoading && (
